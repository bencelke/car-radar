...
import { FollowingPageContent } from "@/components/following/FollowingPageContent";
import { useAuth } from "@/components/providers/AuthProvider";

export function FollowingPageGate() {
  ...
  if (!user) {
    return (
      <div className="flex min-h-[50vh] flex-col items-center justify-center gap-2 px-4 text-center">
        ...
        <Link href="/login?next=/following" className="text-sm text-[#3B82F6] hover:underline">
          Sign in
        </Link>
      </div>
    );
